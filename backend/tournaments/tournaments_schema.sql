...
create table if not exists public.music_tournament_ballots (
  tournament_id uuid not null references public.music_tournaments(id) on delete cascade,
  user_id uuid not null references auth.users(id) on delete cascade,
  picks jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
...
create index if not exists music_tournament_ballots_user_idx
  on public.music_tournament_ballots (user_id, updated_at desc);
...
alter table public.music_tournament_ballots enable row level security;
...
drop policy if exists "music_tournament_ballots_select_own" on public.music_tournament_ballots;
create policy "music_tournament_ballots_select_own"
on public.music_tournament_ballots
for select
to authenticated
using (auth.uid() = user_id);

drop policy if exists "music_tournament_ballots_insert_own" on public.music_tournament_ballots;
create policy "music_tournament_ballots_insert_own"
on public.music_tournament_ballots
for insert
to authenticated
with check (
  ...
  and exists (
    select 1
    from public.music_tournaments t
    where t.id = music_tournament_ballots.tournament_id
      and (
        t.visibility = 'public'
        or t.owner_id = auth.uid()
        or exists (
...
drop policy if exists "music_tournament_ballots_update_own" on public.music_tournament_ballots;
create policy "music_tournament_ballots_update_own"
on public.music_tournament_ballots
for update
to authenticated
using (auth.uid() = user_id)
...
create or replace function public.set_my_music_tournament_ballot(target_tournament_id uuid, next_picks jsonb)
returns boolean
language plpgsql
security definer
set search_path = public
...
begin
  ...
  values (target_tournament_id, me, coalesce(next_picks, '{}'::jsonb))
...
grant execute on function public.set_my_music_tournament_ballot(uuid, jsonb) to authenticated;
...
set search_path = public
as $$
  select public.set_my_music_tournament_ballot(target_tournament_id, next_picks);
$$;
